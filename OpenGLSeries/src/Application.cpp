...
Application::Application(const std::string& name)
{
	...
	m_Window->SetEventCallback(SET_EVENT_FN(onEvent));
...
void Application::onEvent(Event& e)
{
	...
	dispatcher.dispatch<WindowCloseEvent>(SET_EVENT_FN(onWindowClose));
	dispatcher.dispatch<KeyPressedEvent>(SET_EVENT_FN(onKeyPressed));
	//dispatcher.dispatch<KeyReleasedEvent>(SET_EVENT_FN(onKeyReleased));
	LOG_INFO("{0}", e);
}
