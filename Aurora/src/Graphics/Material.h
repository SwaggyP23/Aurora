...
namespace Aurora {
	...
	public:
		...
		Ref<CubeTexture> TryGetCubeTexture(std::string_view name);

		uint32_t GetFlags() const { return m_MaterialFlags; }
		bool HasFlag(MaterialFlag flag) const { return (uint32_t)flag & (uint32_t)m_MaterialFlags; }
		void SetFlag(MaterialFlag flag, bool value);

		Ref<Shader> GetShader() const { return m_Shader; }
		const std::string& GetName() const { return m_Name; }
		const Buffer& GetUniformBufferStorage() const { return m_UniformStorageBuffer; }
	...
	public:
		...
		Ref<MaterialAsset> GetMaterial(uint32_t materialIndex) const { AR_CORE_ASSERT(HasMaterial(materialIndex)); return m_Materials.at(materialIndex); }
