...
namespace ImGui {
    ...
    bool TreeNodeWithIcon(Aurora::Ref<Aurora::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint)
    {
        ...
        const ImU32 arrow_col = selected ? Theme::NiceBlue : Theme::Accent;
        ...
        if (display_frame)
        {
            ...
            if (icon)
            {
                ...
                ImGui::Image(textureID, ImVec2{ iconSize, iconSize }, ImVec2{ 0, 0 }, ImVec2{ 1, 1 }, iconTint);
    ...
    bool TreeNodeWithIcon(Aurora::Ref<Aurora::Texture2D> icon, const void* ptr_id, ImGuiTreeNodeFlags flags, ImColor iconTint, const char* fmt, ...)
	{
        ...
        ImGuiContext& g = *GImGui;
        const char* label_end = &g.TempBuffer[0] + ImFormatStringV(&g.TempBuffer[0], IM_ARRAYSIZE(&g.TempBuffer[0]), fmt, args);

        bool is_open = TreeNodeWithIcon(icon, window->GetID(ptr_id), flags, &g.TempBuffer[0], label_end, iconTint);

        va_end(args);
        return is_open;
	}

    bool TreeNodeWithIcon(Aurora::Ref<Aurora::Texture2D> icon, const char* label, ImGuiTreeNodeFlags flags, ImColor tintColor)
    {
        ImGuiWindow* window = ImGui::GetCurrentWindow();
        if (window->SkipItems)
            return false;

        return TreeNodeWithIcon(icon, window->GetID(label), flags, label, NULL, tintColor);
    }
